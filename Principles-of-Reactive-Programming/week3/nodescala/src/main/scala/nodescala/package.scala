...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      p.success(Nil)
      fs.foldLeft(p.future)((acc, cur) => for (x <- acc; y <- cur) yield x++List(y))
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]()
      for (e <- fs)
        e.onComplete { x => p.tryComplete(x) }
      p.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = f.value match {
      case Some(x) => x match {
        ...
        case Failure(res) => throw res
      }
      case None => throw new NoSuchElementException("Has not finished yet")
    }
     ...
     *  The resulting future contains a value returned by `cont`.
     */
    def continue[S](cont: Try[T] => S): Future[S] = Future {
      Await.result(f, Duration.Inf)
      cont(f.value.get)
    }
