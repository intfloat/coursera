...
# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = wordsGrouped.map(lambda pair: (pair[0], len(pair[1])))
...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = wordsRDD.map(lambda w: (w, 1)).groupByKey().count()
print uniqueWords
...
# TODO: Replace <FILL IN> with appropriate code
shakeWordsRDD = shakespeareWordsRDD.filter(lambda w: len(w) > 0)
...
# TODO: Replace <FILL IN> with appropriate code
top15WordsAndCounts = wordCount(shakeWordsRDD).takeOrdered(15, lambda pair: -pair[1])
