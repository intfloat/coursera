...
URL = 'webapplications-001'

# the "Identifier" you used when creating the part
partIds = ['ba1']
# used to generate readable run-time information for students
partFriendlyNames = ['Blog Iteration 1']
# source files to collect (just for our records)
sourceFiles = ['input_bitbucket_url.txt']
          
def output(partIdx):
  """Uses the student code to compute the output for test cases."""
  outputString = ''
      #  if partIdx == 1: # This is degree-part-4
  infile = open('input_bitbucket_url.txt')  ## Read in input
  for line in infile:
    outputString += line
