...
% 50 variables, 1000 pairs for ranking constraint
% preferences is a 1000*2 matrix

fprintf('Solving ranking problem using quadratic cost function...\n');

% for part a, use quadratic cost function
cvx_begin
    variables r(50)
    cvx_quiet(true)
    cost = 0;
    for i = 1:m
        a = preferences(i, 1);
        b = preferences(i, 2);
        cost = cost+pow_pos(max(0, r(b)+1-r(a)), 2);
    end
    minimize(cost)
    subject to
        r >= 0
        % set a baseline for ranking
        r(1) == 0
cvx_end

fprintf('Problem status: %s\n', cvx_status);
fprintf('Minimal cost is: %f\n', cost);

% calculate number of violated ranking constraints.
violation_num = 0;
for i = 1:m
    a = preferences(i, 1);
    b = preferences(i, 2);
...
fprintf('Number of violated ranking constraints is: %d\n', violation_num);

fprintf('Solving ranking problem using linear cost function...\n');
...
% 881 vs 332 violations when 0.0001
